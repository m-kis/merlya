...
class GeneralConfig(BaseModel):
    ...
    data_dir: Path = Field(
        default=Path.home() / ".merlya", description="Data directory path"
    )
...
class RouterConfig(BaseModel):
    ...
    type: Literal["local", "llm"] = Field(
        default="local", description="Router type"
    )
    model: str | None = Field(
        default=None, description="Local embedding model ID"
    )
    tier: str | None = Field(
        default=None, description="Model tier (performance, balanced, lightweight)"
    )
    llm_fallback: str = Field(
        default="openai:gpt-4o-mini", description="LLM fallback for routing"
    )
...
class SSHConfig(BaseModel):
    ...
    pool_timeout: int = Field(
        default=600, ge=60, le=3600, description="Pool timeout in seconds"
    )
    ...
    default_user: str | None = Field(
        default=None, description="Default SSH username"
    )
    default_key: Path | None = Field(
        default=None, description="Default private key path"
    )
...
class UIConfig(BaseModel):
    ...
    theme: Literal["auto", "light", "dark"] = Field(
        default="auto", description="Color theme"
    )
    markdown: bool = Field(
        default=True, description="Enable markdown rendering"
    )
    syntax_highlight: bool = Field(
...
class LoggingConfig(BaseModel):
    ...
    max_files: int = Field(
        default=5, ge=1, le=20, description="Max number of log files"
    )
    retention_days: int = Field(
        default=7, ge=1, le=90, description="Log retention in days"
    )
