...
class TestSubagentFactory:
    ...
    def test_create_with_skill(self, mock_context):
        """Test creating subagent with skill configuration."""
        ...
        skill = SkillConfig(
            ...
            tools_allowed=["ssh_execute", "check_disk_usage"],
            system_prompt="You are a disk audit specialist.",
        )
        ...
        assert instance.skill_name == "disk_audit"
        assert "ssh_execute" in instance.tools_allowed
        assert "disk audit specialist" in instance.system_prompt

    def test_create_with_tools_override(self, mock_context):
        """Test that explicit tools override skill tools."""
        from merlya.skills.models import SkillConfig
        from merlya.subagents.factory import SubagentFactory
        ...
        assert instance.tools_allowed == ["custom_tool"]

    def test_build_system_prompt_with_tools_guidance(self, mock_context):
        """Test that tools guidance is added to system prompt."""
        from merlya.skills.models import SkillConfig
        from merlya.subagents.factory import SubagentFactory
        ...
        assert "Allowed Tools" in instance.system_prompt
        assert "ssh_execute" in instance.system_prompt
        assert "read_file" in instance.system_prompt
        assert "ONLY use these tools" in instance.system_prompt
...
class TestSkillExecutorIntegration:
    """Tests for SkillExecutor integration with subagents."""
    ...
    def test_executor_has_real_execution_with_context(self, mock_context):
        """Test that executor has real execution when context is provided."""
        from merlya.skills.executor import SkillExecutor

        executor = SkillExecutor(context=mock_context)

        assert executor.has_real_execution is True
        assert executor.orchestrator is not None

    def test_executor_no_real_execution_without_context(self):
        """Test that executor uses simulation without context."""
        from merlya.skills.executor import SkillExecutor
    ...
    @pytest.mark.asyncio
    async def test_executor_uses_orchestrator(self, mock_context):
        """Test that executor uses orchestrator when available."""
        from merlya.skills.executor import SkillExecutor
        from merlya.skills.models import SkillConfig

        executor = SkillExecutor(context=mock_context, max_concurrent=2)

        skill = SkillConfig(name="test_skill", max_hosts=5, timeout_seconds=30)

        # Mock the orchestrator's run_on_host
        mock_result = MagicMock()
        mock_result.success = True
        mock_result.output = "Test output"
        mock_result.error = None
        mock_result.duration_ms = 1000
        mock_result.tool_calls = 2

        with patch.object(
            executor.orchestrator, "run_on_host", AsyncMock(return_value=mock_result)
        ):
    ...
    @pytest.mark.asyncio
    async def test_executor_fallback_to_simulation(self):
        """Test that executor falls back to simulation without context."""
        from merlya.skills.executor import SkillExecutor
        from merlya.skills.models import SkillConfig

        executor = SkillExecutor()  # No context
