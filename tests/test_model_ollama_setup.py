...
class TestOllamaModelSetup:
    ...
    def test_handle_ollama_model_setup_user_declines_download(
        self, mock_get_client, mock_prompt, model_handler
    ):
        ...
        mock_get_client.return_value = mock_client
        mock_prompt.return_value = "n"  # User says no
    ...
    def test_handle_ollama_model_setup_download_success(
        ...
        mock_get_client.return_value = mock_client
        mock_prompt.return_value = "y"  # User says yes
    ...
    @patch('prompt_toolkit.prompt')
    @patch('athena_ai.llm.ollama_client.get_ollama_client')
    def test_handle_ollama_model_setup_prompt_unavailable(
        self, mock_get_client, mock_prompt, model_handler
    ):
        """Should handle gracefully if prompt_toolkit is not available."""
        mock_client = MagicMock()
        mock_client.is_available.return_value = True
        mock_client.has_model.return_value = False
        mock_get_client.return_value = mock_client
        mock_prompt.side_effect = ImportError("prompt_toolkit not available")
