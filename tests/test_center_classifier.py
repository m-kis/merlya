...
class TestDiagnosticPatterns:
    ...
    async def test_diagnostic_inputs(
...
class TestAmbiguousInputs:
    ...
    async def test_low_confidence_needs_clarification(
        self, classifier: CenterClassifier
    ) -> None:
...
class TestMixedIntents:
    ...
    async def test_check_and_fix_is_change(
        self, classifier: CenterClassifier
    ) -> None:
    ...
    async def test_pure_question_is_diagnostic(
        self, classifier: CenterClassifier
    ) -> None:
...
class TestClarificationPrompts:
    ...
    def test_generates_clarification_for_diagnostic(
        self, classifier: CenterClassifier
    ) -> None:
    ...
    def test_generates_clarification_for_change(
        self, classifier: CenterClassifier
    ) -> None:
...
class TestPatternCounting:
    ...
    async def test_multiple_diagnostic_patterns(
        self, classifier: CenterClassifier
    ) -> None:
        """Test multiple diagnostic patterns increase confidence."""
        result = await classifier.classify(
            "check disk usage, show logs, and analyze memory usage"
        )
        assert result.center == CenterMode.DIAGNOSTIC
        assert result.confidence >= 0.7

    async def test_multiple_change_patterns(
        self, classifier: CenterClassifier
    ) -> None:
        """Test multiple change patterns increase confidence."""
        result = await classifier.classify(
            "restart nginx, update config, and deploy the app"
        )
        assert result.center == CenterMode.CHANGE
        assert result.confidence >= 0.7
